...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Dislike request body:", body);
    ...
    if (!body.target_type || !body.target_id) {
      console.log("Missing required fields:", { target_type: body.target_type, target_id: body.target_id });
      return NextResponse.json({ message: "target_type과 target_id는 필수입니다." }, { status: 400 });
    }

    if (!isValidTargetType(body.target_type)) {
      console.log("Invalid target type:", body.target_type);
      return NextResponse.json({ message: "유효하지 않은 target_type입니다." }, { status: 400 });
    }

    const response = await interactionApi.addDislike(
      {
    ...
    const errorMessage = error instanceof Error ? error.message : "싫어요 추가 중 오류가 발생했습니다.";
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const response = await interactionApi.removeDislike(target_type, Number(target_id), authHeader);

    return NextResponse.json(response);
  } catch (error) {
    console.error("싫어요 제거 오류:", error);
    const errorMessage = error instanceof Error ? error.message : "싫어요 제거 중 오류가 발생했습니다.";
    return NextResponse.json({ message: errorMessage }, { status: 500 });
  }
}
